...
The number of innovations in each condition by generation (1-6, left to right).  The number of innovations declines in the learning condition:

```{r}
barplot(t(table(
  datax[datax$Human,]$condition,
  datax[datax$Human,]$gen)), beside = T,
...
# innovations produced by the human in the learning condition
...
hist(learn.innovation.iconicity.dist, 
     ...
     border = cols[2])
abline(v=0)
legend(-0.45,150, legend = c("Communication","Learning"), col=cols, pch=15, cex=0.8)
...
hist(learn.innovation.sys.dist, 
     ...
     border = NA)
abline(v=0)
legend(-0.3,250, legend = c("Communication","Learning"), col=cols, pch=15)
```
...
```{r}
sjp.lmer(m3,'fe', geom.colors = c(1,1))
```
